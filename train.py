...
class TrainerWithGeneration(transformers.Seq2SeqTrainer):
    ...
    def evaluation_loop(
        ...
        eval_distance(
            all_pred, all_label, "speed_error", r"speed is (\d+(?:\.\d+)?) mph"
        )
        eval_distance(
            all_pred, all_label, "steer_error", r"Steering wheel is (\d+(?:\.\d+)?)%"
        )
        eval_distance(
            all_pred, all_label, "path_error", r"turning (\d+(?:\.\d+)?) degrees"
        )
...
def train(
    ...
    data_path: str = "vqa_train_8k.pkl",
    ...
    resume_from_checkpoint: str = "pretrained_model/",  # always resume from pre-finetuned alpaca model
    augment_times: int = 100,
    output_dir: Optional[str] = None,
    vqa: bool = False,
    eval_items: List[str] = DEFAULT_EVAL_ITEMS,
    mode: str = "train",
    load_pre_prompt_dataset: bool = False,
    val_data_path: str = "test_1k.pkl",
):
